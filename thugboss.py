...
# Updated with your new Quiz Cash JSON URL
...
# Updated with your new Key and Project ID
...
REQUEST_TIMEOUT = 30
# Port updated to avoid "Address already in use" errors
PORT = 10100 
...
async def load_config(client):
    ...
    # Filter to remove control characters that cause JSON errors
        ...
        while True:
            try:
                ...
                if not offer:
                    ...
                    await asyncio.sleep(60) # Keep running between rounds
                    continue

                log(f"Target Found: {offer['offerId']} | Boosts needed: {offer['fees']}")
                for i in range(offer["fees"] + 1):
                    await run_fairbid(client, cfg)
                    if i % 5 == 0: log(f"Boost Progress: {i}/{offer['fees']}")
                    await asyncio.sleep(0.5) # Stable reward speed
